...
# Общие улучшения
 ...
 3. Добавлен `front_controller` для считыватия скрытого поля `_method` что позволило использовать дополнительно `PUT`, `PATCH`, `DELETE` запросы (необходимые в том числе для API).
 4. Используется перенаправление на страницу методом фреймфорка. 
